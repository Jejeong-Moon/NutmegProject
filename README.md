...
- 닉네임은 중복 검사를 통해 유일하게 유지된다.
- 가입 시 USER_CODE 테이블이 생성되고 USER_INFO 테이블에 데이터가 등록되어 회원탈퇴시 정보를 유지할 수 있게 구현.

### 🏃 동호회 생성 및 가입
- 사용자는 임시 동호회를 생성할 수 있으며, 관리자의 승인을 받으면 정식 동호회가 된다.
- 다른 사용자는 생성된 임시 동호회에 가입 신청할 수 있고, 운영자의 승인을 통해 팀원이 된다.
...
- 팀에 인원이 부족할 경우 용병을 초대해 경기를 진행할 수 있다.
